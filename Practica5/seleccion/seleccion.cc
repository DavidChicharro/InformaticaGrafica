...
typedef enum{GRUA,PIRAMIDES,CUBO} _tipo_objeto;
...
{
	switch (t_objeto){
		//case GRUA: grua.draw(mode,0.0,0.0,1.0,0.0,1.0,1.0,2);break;
		case GRUA:
			glPushMatrix();
...
void normal_keys(unsigned char Tecla1,int x,int y)
{
	switch (toupper(Tecla1)){
		case 'Q': exit(0);
		case '1':mode=POINTS;break;
		case '2':mode=EDGES;break;
		case '3':mode=SOLID;break;
		case '4':mode=SOLID_CHESS;break;
			case 'G':t_objeto=GRUA;break;
			case 'P':t_objeto=PIRAMIDES;break;
...
void procesar_hits(GLint hits, GLuint *names)
 ...
 void procesar_hits_2(GLint hits, GLuint *names)
 {
 	int i;
   _cubo* obj = nullptr;

        //obj=(_grua )malloc(sizeof(_grua));
